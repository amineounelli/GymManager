...
public boolean modifierSeanceIndividuelle(SeanceIndividuelle s) {
    String sql = """
        UPDATE seance SET 
            duree=?, date_seance=?, cout_horaire=?, salle_id=?, membre_id=?
        WHERE id_seance=?
    """;
    ...
    try (Connection conn = DatabaseConnection.getConnection();
        ...
        p.setDouble(1, s.getDuree());
        p.setDate(2, new java.sql.Date(s.getDate().getTime()));
        p.setDouble(3, s.getCoutHoraire());
        p.setInt(4, s.getSalle().getIdSalle());
        p.setInt(5, s.getMembre().getId());
        p.setInt(6, s.getIdSeance());
...
// UPDATE COLLECTIVE SESSION
public boolean modifierSeanceCollective(SeanceCollective s) {
    String sql = """
        UPDATE seance SET 
            duree=?, date_seance=?, cout_horaire=?, salle_id=?, coach_id=?, capacite=?
        WHERE id_seance=?
    """;
    ...
    try (Connection conn = DatabaseConnection.getConnection();
        ...
        p.setDouble(3, s.getCoutHoraire());
        p.setInt(4, s.getSalle().getIdSalle());
        p.setInt(5, s.getCoach().getId());
        p.setInt(6, s.getCapacite());
        p.setInt(7, s.getIdSeance());
