...
public class ManagerDAO {
    ...
    String sql = "UPDATE personne p " +
                 "INNER JOIN manager m ON p.id = m.id " +
                 "SET p.nom = ?, p.prenom = ?, p.email = ?, p.telephone = ?, " +
                 "    m.login = ?, m.mot_de_passe = ? " +
                 "WHERE p.id = ?";

    try (Connection conn = DatabaseConnection.getConnection();
         PreparedStatement pst = conn.prepareStatement(sql)) {

        pst.setString(1, manager.getNom());
        pst.setString(2, manager.getPrenom());
        pst.setString(3, manager.getEmail());
        pst.setString(4, manager.getTelephone());
        pst.setString(5, manager.getLogin());
        pst.setString(6, manager.getMotDePasse());
        pst.setInt(7, manager.getId());

        int rows = pst.executeUpdate();
        return rows > 0;

    } catch (SQLException e) {
        System.err.println("❌ Erreur lors de la modification du manager: " + e.getMessage());
        e.printStackTrace();
        return false;
    }
}
